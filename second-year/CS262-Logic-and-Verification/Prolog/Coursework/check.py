...
formulas = ["p(a)", "q(a)"]
premises = ["[forall(X, p(X))]", "[forall(X, p(X))]"]
...
def run_tests():
    ...
    for i in range(0, len(formulas)):
        ...
        p = premises[i]
        copyfile(
            path + "resolution.pl", "test.pl"
        )  # copy test file to current directory
        file = open("test.pl", "a")
        file.write(
            "\n\nmy_test:-test(" + p + ", " + f + ")."
        )  # append test commands to this Prolog file
        file.close()
        ...
        file.close()
        answers = re.findall(
            r"YES|NO", fstr, re.IGNORECASE
        )  # search for occurrences of yes/no in the output file
        print(answers)
        if len(answers) == 0:
        ...
        if len(answers) > 1:
            print(
                "  WARNING: multiple answers given for f=" + f
            )  # take the last one in this case
