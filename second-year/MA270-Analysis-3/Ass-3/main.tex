...
Since the operator norm is a norm, $\opnorm{A^{n+1}} = \opnorm A \opnorm{A^n}$ for any $n \in \N$. Therefore $\opnorm{A^n} = {\opnorm A}^n$. We know that $\opnorm A < 1$, so $\opnorm{A^n} \to 0$ as $n \to \infty$.
