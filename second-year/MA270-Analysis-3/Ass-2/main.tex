...
We shall consider instead $\sum_{n=1}^\infty |f_n|$. % TODO
...
Yes, it converges to 0. We want to show that $\forall \varepsilon > 0$, $\exists \delta > 0$ such that $\| (x, y) - (0, 0) \| < \delta \implies | f(x, y) - 0 | < \varepsilon$. That is, we want $\sqrt{x^2 + y^2} < \delta \implies | f(x, y) | < \varepsilon$. Let us assume $\delta < 1$.
%
\begin{align*}
...
Idk. Probably not, as per question structure.

We will first show that $|\!\sin x| \le |x|$ for all $x \in \R$. Clearly $0 \le |\!\sin x| \le 1$ for all $x$, so we only need to consider $-1 \le x \le 1$ carefully. % TODO

\subsection{~} % 4.d

It converges to 0. $\sin\l(\f1{2x+3y}\r)$ doesn't have a limit at the origin, but it will oscillate between $-1$ and $1$ with increasing frequency as $(x, y) \to (0, 0)$. However, $3x+2y$ converges linearly to 0 as $(x, y) \to (0, 0)$, and so this term will dominate $f$. Therefore $f(x, y) \to 0$ as $(x, y) \to (0, 0)$.
% TODO: Explain better
