...
\centering
\begin{tikzpicture}
    \node[label=above:$P$] (P) at (0, 1.8) {};
    \node[label=left:$M$] (M) at (-1.8, 0) {};
    \node[label=right:$N$] (N) at (1.8, 0) {};
    \node[label=below:$\exists T$] (T) at (0, -1.8) {};

    \fill (P) circle[radius=0.15];
    \fill (M) circle[radius=0.15];
    \fill (N) circle[radius=0.15];
    \draw (T) circle[radius=0.15];
...
It would be nice if we could break this down into a series of $\beta$-contractions and do some diagram chasing, as seen in Figure~\ref{fig:beta-contraction-diagram-chasing}.
...
The idea of the Tait--Martin-L\"of proof is to define a new relation $\pred$ (parallel reduction) whose transitive closure is the same as that of $\bred$. Then if $\pred$ satisfies the diamond property, then so does $\bred$ and we have proven the Church--Rosser Theorem. % chktex 8

\begin{defn}
We will define a new binary relation $\pred$ on $\lambda$-terms as follows:
\begin{enumerate}[(1)]
\item $M \pred M$;
\item if $M \pred M'$ then $\lambda x . M \pred \lambda x . M'$;
\item if $M \pred M'$ and $N \pred N'$ then $MN \pred M' N'$;
\item if $M \pred M'$ and $N \pred N'$ then $(\lambda x . M) N \pred M' [x := N']$.
\end{enumerate}
\end{defn}

\begin{lemma}
If $M \pred M'$ and $N \pred N'$ then $M [x := N] \pred M' [x := N']$.
\end{lemma}

\begin{proof}
By induction on the definition of $M \pred M'$. There are 4 cases to consider.

\begin{case}
$M \pred M'$ is $M \pred M$. % TODO
\end{case}

\begin{case}
$M \pred M'$ is $\lambda y . P \pred \lambda y . P' \pred M$.
\end{case}
