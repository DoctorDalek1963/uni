...
% TODO: Explain Currying better
% TODO: Motivate with analogy of functions
% TODO: Motivate choice of Church numerals
...
% TODO: Explain some interesting consequences, especially anything that connects this stuff to other parts of mathematics or computer science.  Why did Church and colleagues pursue these ideas?  How, more precisely, do they relate to a real-world programming language?  On the other hand, can you expand Section 4 to explain how these formal definitions cover set theory more broadly, beyond just arithmetic.
...
We want $\lambda$-calculus to be confluent because we want one term to always produce the same result, no matter what order its $\beta$-redexes were contracted. $\overline 1 + \overline 1$ should always $\beta$-reduce to $\overline 2$, regardless of the inner workings of the reduction. If $\lambda$-calculus is confluent, it would allow us to ignore these inner workings for much of the theoretical work that we want to do with the system.
...
The rest of the proof is tedious and fairly uninteresting. The idea is that every $\alpha$-conversion or $\beta$-contraction is a special case of parallel reduction. Then to prove Theorem~\ref{thm:church-rosser-beta-reduction}, we observe that $P \pred M$ and $P \pred N$,
