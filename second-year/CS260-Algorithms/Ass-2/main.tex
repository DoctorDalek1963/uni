...
\textsc{Santa-Service} requires finding a Hamiltonian path, so if we can show that \textsc{Hamiltonian-Path} is NP-hard then \textsc{Santa-Service} is NP-hard as well.
...
% Give a restriction of the input under which \textsc{Santa-Service} is solvable in polynomial time, but not in constant time.
Answer
